...
CREATE POLICY IF NOT EXISTS clients_select_own ON clients
  FOR SELECT USING (auth.uid() = user_id OR is_guest = true);
  
-- Policy para insertar datos de clientes
CREATE POLICY IF NOT EXISTS clients_insert ON clients
  FOR INSERT WITH CHECK (true);
  
-- Policy para actualizar datos de clientes
CREATE POLICY IF NOT EXISTS clients_update ON clients
